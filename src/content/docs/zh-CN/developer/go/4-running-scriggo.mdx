...
本页讲**阶段二 · Scriggo VM（运行期）**：宿主在运行时如何驱动你在 [原生 Go 运行](/developer/go/3-running-native/) 中写好的扩展。
...
- **只能使用后端已有的包**：扩展里 `import` 的包必须是 miru-core 后端已使用并在 `packages.go` 导出的（见 [详细用法](/developer/go/2-detail-usage/) 的「可导入的包」）。引入未在 `packages.go` 注册的包会编译失败。
- **受 Scriggo 限制约束**：方法声明、接口定义、部分 `for range` 写法等不被支持，详见 [详细用法](/developer/go/2-detail-usage/) 的「Scriggo 的限制」。
...
- **编译错误**：源文件无法被 Scriggo 编译，错误信息以 `compile extension <包名>: ...` 开头。原因通常是：
  - 使用了 Scriggo 不支持的语法（方法声明、接口定义、带标签的 `continue/break`、特定 `for range` 写法等，见 [详细用法](/developer/go/2-detail-usage/) 的「Scriggo 的限制」）。
...
- 如果阶段一通过、阶段二报错：几乎一定是 **Scriggo 限制或未导出的包**问题（语法不支持 / 引入了未在 `packages.go` 注册的包 / 寄存器或类型上限），对照 [详细用法](/developer/go/2-detail-usage/) 逐条排查。
...
:::tip
由于阶段一与阶段二共用源码、返回结构一致，你在 [原生 Go 运行](/developer/go/3-running-native/) 用 `go test` 验证通过的扩展，放进 Miru 后几乎不会出现运行期差异。若运行期报错，优先检查是否使用了 Scriggo 不支持的语法或引入了未在 `packages.go` 注册的包。
:::
