...
This page defines the **input parameters and return structures** of each extension entry point, and corresponds one-to-one with the entry functions in [JavaScript Extensions (V2)](/docs/en/developer/js/1-get-started/) / [Go Extensions](/docs/en/developer/go/1-get-started/).
...
- **V1 (legacy-compatible)**: `watch()` returns the final playback shape directly, with no `mirror()` step — see [Data Formats (V1, legacy)](#data-formats-v1-legacy) at the bottom. Reference only for existing extensions; [JavaScript Extensions V1 (legacy)](/docs/en/developer/js/2-legacy-v1/) covers its syntax.
...
:::caution
V1 is no longer recommended for new extensions; new extensions should use V2 uniformly (see [JavaScript Extensions](/docs/en/developer/js/1-get-started/) and [Go Extensions](/docs/en/developer/go/1-get-started/)).
:::
