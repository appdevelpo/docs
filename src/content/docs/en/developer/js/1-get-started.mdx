...
Miru Alpha's JavaScript extensions run on the pure-Go [goja](https://github.com/dop251/goja) engine; extension files use the `.js` suffix. This page covers the **V2 (recommended)** JS workflow; see [Getting Started](/en/developer/2-extension/) for the shared metadata and [Data Formats](/en/developer/5-data/) for the return structures.

:::caution[Legacy V1 is no longer recommended]
Extensions still using the old `class extends Extension` style should refer to [JavaScript Extensions V1 (legacy)](/en/developer/js/2-legacy-v1/). **New extensions should use the V2 API on this page**: `watch()` returns a list of mirrors and `mirror()` resolves the final URL, matching the Go extension contract.
:::
