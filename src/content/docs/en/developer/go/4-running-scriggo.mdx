...
This page covers **Stage 2 · Scriggo VM (runtime)**: how the host drives the extension you wrote in [Running with Native Go](/en/developer/go/3-running-native/).
...
- **Only packages the backend already uses are importable**: packages you `import` must be part of the miru-core binary and exported in `packages.go` (see "Importable Packages" in [Detail Usage](/en/developer/go/2-detail-usage/)). Importing a package that is not registered in `packages.go` fails compilation.
- **Scriggo limitations apply**: method declarations, interface definitions, some `for range` forms, etc. are unsupported — see "Scriggo Limitations" in [Detail Usage](/en/developer/go/2-detail-usage/).
...
- **Compile errors**: the source can't be compiled by Scriggo; the message starts with `compile extension <pkg>: ...`. Typical causes:
  - Scriggo-unsupported syntax (method declarations, interface definitions, labeled `continue`/`break`, certain `for range` forms — see "Scriggo Limitations" in [Detail Usage](/en/developer/go/2-detail-usage/)).
...
- If Stage 1 passes but Stage 2 errors: it's almost certainly a **Scriggo limitation or un-exported package** issue (unsupported syntax / un-registered import / register-or-type cap) — check it against [Detail Usage](/en/developer/go/2-detail-usage/) point by point.
...
:::tip
Because Stage 1 and Stage 2 share source and return shapes, an extension you verified with `go test` in [Running with Native Go](/en/developer/go/3-running-native/) will rarely differ at runtime. If a runtime error appears, first check for Scriggo-unsupported syntax or an import not registered in `packages.go`.
:::
